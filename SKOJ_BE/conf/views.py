...
class SlideshowsAPI(APIView):
    def get(self, request):
        slideshows = []
        for i in SysOptions.slideshows:
            if i['visible']:
                slideshows.append(i)
        return self.success(slideshows)

class SlideshowsAdminAPI(APIView):
    @super_admin_required
    def get(self, request):
        return self.success(SysOptions.slideshows)

    @super_admin_required
    def post(self, request):
        slideshows = request.data.get("slideshows")
        setattr(SysOptions, "slideshows", slideshows)
        return self.success()

class SoftwaresAPI(APIView):
    def get(self, request):
        sws = []
        for i in SysOptions.softwares:
            if i['visible']:
                sws.append(i)
        return self.success(sws)

class SoftwaresAdminAPI(APIView):
    @super_admin_required
    def get(self, request):
        return self.success(SysOptions.softwares)

    @super_admin_required
    def post(self, request):
        softwares = request.data.get("softwares")
        setattr(SysOptions, "softwares", softwares)
        return self.success()

class ClearSWPicCached(APIView):
    @super_admin_required
    def delete(self, request):
        softwares = SysOptions.softwares
        usedfiles = []
        for i in softwares:
            usedfiles.append(os.path.basename(i['pic']))
        files = os.listdir(settings.SOFTWARE_UPLOAD_DIR)
        for i in files:
            if i not in usedfiles and 'gitkeep' not in i:
                os.remove(os.path.join(settings.SOFTWARE_UPLOAD_DIR, i))
...
class ReleaseNotesAPI(APIView):
    def get(self, request):
        with open("docs/data.json", "r", encoding='utf-8') as f:
            ...
            return self.success(local_version)
