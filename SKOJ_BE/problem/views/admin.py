...
from submission.models import Submission, JudgeStatus, ObjectiveSubmission
from utils.api import APIView, CSRFExemptAPIView, validate_serializer, APIError
from utils.constants import Difficulty, TMP_PATH
from utils.shortcuts import rand_str, natural_sort_key
from utils.tasks import delete_files
from ..models import Problem, ProblemRuleType, ProblemTag, ObjectiveProblemTag, ObjectiveProblem
from ..serializers import (CreateContestProblemSerializer, CompileSPJSerializer, ContestObjectiveProblemMakePublicSerializer,
                           CreateProblemSerializer, EditProblemSerializer, EditContestProblemSerializer,
                           ProblemAdminSerializer, TestCaseUploadForm, ContestProblemMakePublicSerializer,
                           ObjectiveProblemAdminSerializer,CreateObjectiveProblemSerializer,EditObjectiveProblemSerializer,
                           AddContestProblemSerializer, ExportProblemSerializer, CreateContestObjectiveProblemSerializer,
                           EditContestObjectiveProblemSerializer, AddContestObjectiveProblemSerializer,
                           ExportProblemRequestSerialzier, UploadProblemForm, ImportProblemSerializer,
                           ExportObjectiveProblemSerializer, ExportObjectiveProblemRequestSerialzier,
...
class TestCaseAPI(CSRFExemptAPIView, TestCaseZipProcessor):
    ...
    def post(self, request):
        ...
        zip_file = TMP_PATH + f"{rand_str()}.zip"
...
class ObjectiveProblemAPI(APIView):
    @problem_permission_required
    @validate_serializer(CreateObjectiveProblemSerializer)
    def post(self, request):
        data = request.data
        _id = data["_id"]
        if not _id:
            return self.error("Display ID is required")
        if ObjectiveProblem.objects.filter(_id=_id, contest_id__isnull=True).exists():
            return self.error("Display ID already exists")

        # todo check filename and score info
        tags = data.pop("tags")
        data["created_by"] = request.user
        problem = ObjectiveProblem.objects.create(**data)

        for item in tags:
            try:
                tag = ObjectiveProblemTag.objects.get(name=item)
            except ObjectiveProblemTag.DoesNotExist:
                tag = ObjectiveProblemTag.objects.create(name=item)
            problem.tags.add(tag)
        return self.success(ObjectiveProblemAdminSerializer(problem).data)
    ...
    def get(self, request):
        ...
        if problem_id:
            try:
                problem = ObjectiveProblem.objects.get(id=problem_id)
                ensure_created_by(problem, request.user)
                return self.success(ObjectiveProblemAdminSerializer(problem).data)
            except ObjectiveProblem.DoesNotExist:
                return self.error("Objective Problem does not exist")

        problems = ObjectiveProblem.objects.filter(contest_id__isnull=True).order_by("-create_time")
        ...
        if not user.can_mgmt_all_problem():
            problems = problems.filter(created_by=user)
        return self.success(self.paginate_data(request, problems, ObjectiveProblemAdminSerializer))

    @problem_permission_required
    @validate_serializer(EditObjectiveProblemSerializer)
    def put(self, request):
        data = request.data
        problem_id = data.pop("id")
        ...
        if not _id:
            return self.error("Display ID is required")
        if ObjectiveProblem.objects.exclude(id=problem_id).filter(_id=_id, contest_id__isnull=True).exists():
            return self.error("Display ID already exists")
        ...
        for tag in tags:
            try:
                tag = ObjectiveProblemTag.objects.get(name=tag)
            except ObjectiveProblemTag.DoesNotExist:
                tag = ObjectiveProblemTag.objects.create(name=tag)
            problem.tags.add(tag)
    ...
    def delete(self, request):
        ...
        if not id:
            return self.error("Invalid parameter, id is required")
        try:
            problem = ObjectiveProblem.objects.get(id=id, contest_id__isnull=True)
        except ObjectiveProblem.DoesNotExist:
            return self.error("Objective Problem does not exists")
        ensure_created_by(problem, request.user)
        # d = os.path.join(settings.TEST_CASE_DIR, problem.test_case_id)
        # if os.path.isdir(d):
        #     shutil.rmtree(d, ignore_errors=True)
...
class ContestObjectiveProblemAPI(APIView):
    @validate_serializer(CreateContestObjectiveProblemSerializer)
    def post(self, request):
        data = request.data
        try:
            contest = Contest.objects.get(id=data.pop("contest_id"))
        ...
        if ObjectiveProblem.objects.filter(_id=_id, contest=contest).exists():
            return self.error("Duplicate Display id")
    ...
    def get(self, request):
        ...
        try:
            ...
            return self.error("Contest does not exist")
        problems = ObjectiveProblem.objects.filter(contest=contest).order_by("-create_time")
        if user.is_admin():
            problems = problems.filter(contest__created_by=user)
        keyword = request.GET.get("keyword")
        if keyword:
    ...
    @validate_serializer(EditContestObjectiveProblemSerializer)
    def put(self, request):
        data = request.data
        user = request.user
        ...
        try:
            problem = ObjectiveProblem.objects.get(id=problem_id, contest=contest)
        except ObjectiveProblem.DoesNotExist:
            return self.error("Problem does not exist")
        ...
        if not _id:
            return self.error("Display ID is required")
        if ObjectiveProblem.objects.exclude(id=problem_id).filter(_id=_id, contest=contest).exists():
            return self.error("Display ID already exists")
    ...
    def delete(self, request):
        ...
        if not id:
            return self.error("Invalid parameter, id is required")
        try:
            problem = ObjectiveProblem.objects.get(id=id, contest_id__isnull=False)
        except ObjectiveProblem.DoesNotExist:
            return self.error("Problem does not exists")
        ensure_created_by(problem.contest, request.user)
        if Submission.objects.filter(problem=problem).exists():
...
class MakeContestObjectiveProblemPublicAPIView(APIView):
    @validate_serializer(ContestObjectiveProblemMakePublicSerializer)
    @problem_permission_required
    def post(self, request):
        data = request.data
        display_id = data.get("display_id")
        if ObjectiveProblem.objects.filter(_id=display_id, contest_id__isnull=True).exists():
            return self.error("Duplicate display ID")
        try:
            objective_problem = ObjectiveProblem.objects.get(id=data["id"])
        except ObjectiveProblem.DoesNotExist:
            return self.error("Objective Problem does not exist")
        if not objective_problem.contest or objective_problem.is_public:
            return self.error("Already be a public problem")
        objective_problem.is_public = True
        objective_problem.save()
        # https://docs.djangoproject.com/en/1.11/topics/db/queries/#copying-model-instances
        tags = objective_problem.tags.all()
        objective_problem.pk = None
        objective_problem.contest = None
        objective_problem._id = display_id
        objective_problem.visible = False
        objective_problem.submission_number = 0
        objective_problem.accepted_number = 0
        objective_problem.save()
        objective_problem.tags.set(tags)
        return self.success()
...
class AddContestObjectiveProblemAPI(APIView):
    @validate_serializer(AddContestObjectiveProblemSerializer)
    def post(self, request):
        data = request.data
        try:
            contest = Contest.objects.get(id=data["contest_id"])
            objective_problem = ObjectiveProblem.objects.get(id=data["objective_problem_id"])
        except (Contest.DoesNotExist, ObjectiveProblem.DoesNotExist):
            return self.error("Contest or Objective Problem does not exist")
        ...
        tags = objective_problem.tags.all()
        objective_problem.pk = None
        objective_problem.contest = contest
        objective_problem.is_public = True
        objective_problem.visible = True
        objective_problem._id = request.data["display_id"]
        objective_problem.submission_number = 0
        objective_problem.accepted_number = 0
        objective_problem.save()
        objective_problem.tags.set(tags)
...
class ExportObjectiveProblemAPI(APIView):
    def process_one_problem(self, zip_file, objective_problem, index):
        info = ExportObjectiveProblemSerializer(objective_problem).data
        compression = zipfile.ZIP_DEFLATED
        zip_file.writestr(zinfo_or_arcname=f"{index}/problem.json",
                          data=json.dumps(info, indent=4),
                          compress_type=compression)

    @validate_serializer(ExportObjectiveProblemRequestSerialzier)
    def get(self, request):
        objective_problems = ObjectiveProblem.objects.filter(id__in=request.data["objective_problem_id"])
        for objective_problem in objective_problems:
            if objective_problem.contest:
                ensure_created_by(objective_problem.contest, request.user)
            else:
                ensure_created_by(objective_problem, request.user)
        path = f"/tmp/{rand_str()}.zip"
        with zipfile.ZipFile(path, "w") as zip_file:
            for index, objective_problem in enumerate(objective_problems):
                self.process_one_problem(zip_file=zip_file, objective_problem=objective_problem, index=index + 1)
        delete_files.send_with_options(args=(path,), delay=300_000)
        resp = FileResponse(open(path, "rb"))
        resp["Content-Type"] = "application/zip"
        resp["Content-Disposition"] = "attachment;filename=problem-export.zip"
...
class ImportProblemAPI(CSRFExemptAPIView, TestCaseZipProcessor):
    ...
    def post(self, request):
        ...
        if form.is_valid():
            file = form.cleaned_data["file"]
            tmp_file = TMP_PATH + f"{rand_str()}.zip"
                ...
                for i in range(1, count + 1):
                    with zip_file.open(f"{i}/problem.json") as f:
                        ...
                        problem_obj = Problem.objects.create(_id=problem_info["display_id"],
                                                             ...
                                                             std_code=problem_info["std_code"],
                                                             rule_type=problem_info["rule_type"],
