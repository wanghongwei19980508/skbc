...
from utils.constants import Difficulty, Type
from utils.serializers import LanguageNameMultiChoiceField, SPJLanguageNameChoiceField, LanguageNameChoiceField

from .models import Problem, ProblemRuleType, ProblemTag, ProblemIOMode, ObjectiveProblem, ObjectiveProblemTag
...
class CreateOrEditProblemSerializer(serializers.Serializer):
    ...
    std_code = serializers.DictField(child=serializers.CharField(min_length=1))
...
class CreateOrEditObjectiveProblemSerializer(serializers.Serializer):
    _id = serializers.CharField(max_length=32, allow_blank=True, allow_null=True)
    title = serializers.CharField(max_length=1024)
    description = serializers.CharField()
    ...
    multiple_answer = serializers.JSONField()
    multiple_problem = serializers.JSONField()

class CreateObjectiveProblemSerializer(CreateOrEditObjectiveProblemSerializer):
    pass


class EditObjectiveProblemSerializer(CreateOrEditObjectiveProblemSerializer):
    id = serializers.IntegerField()
...
class CreateContestObjectiveProblemSerializer(CreateOrEditObjectiveProblemSerializer):
    contest_id = serializers.IntegerField()


class EditContestObjectiveProblemSerializer(CreateOrEditObjectiveProblemSerializer):
    id = serializers.IntegerField()
    contest_id = serializers.IntegerField()
...
class ProblemSerializer(BaseProblemSerializer):
    ...
    class Meta:
        ...
        exclude = ("test_case_score", "test_case_id", "visible", "is_public",
                   "spj_code", "spj_version", "spj_compile_ok", "std_code")
...
class ProblemSafeSerializer(BaseProblemSerializer):
    ...
    class Meta:
        ...
        exclude = ("test_case_score", "test_case_id", "visible", "is_public",
                   ...
                   "difficulty", "submission_number", "accepted_number", "statistic_info", "std_code")


class BaseObjectiveProblemSerializer(serializers.ModelSerializer):
    tags = serializers.SlugRelatedField(many=True, slug_field="name", read_only=True)
    created_by = UsernameSerializer()


class ObjectiveProblemAdminSerializer(BaseObjectiveProblemSerializer):
    class Meta:
        model = ObjectiveProblem
        fields = "__all__"


class ObjectiveProblemSerializer(BaseObjectiveProblemSerializer):
    class Meta:
        model = ObjectiveProblem
        exclude = ("visible", "is_public",)

class ObjectiveProblemSafeSerializer(BaseObjectiveProblemSerializer):
    class Meta:
        model = ObjectiveProblem
        exclude = ("visible", "is_public", "answer",
                   "difficulty", "submission_number", "accepted_number")

class ContestProblemMakePublicSerializer(serializers.Serializer):
    id = serializers.IntegerField()
    display_id = serializers.CharField(max_length=32)

class ContestObjectiveProblemMakePublicSerializer(serializers.Serializer):
    id = serializers.IntegerField()
    display_id = serializers.CharField(max_length=32)
...
class ExportProblemSerializer(serializers.ModelSerializer):
    ...
    tags = serializers.SlugRelatedField(many=True, slug_field="name", read_only=True)
    std_code = serializers.SerializerMethodField()
    ...
    def get_std_code(self, obj):
        ret = {}
        for k, v in obj.std_code.items():
            ret[k] = v
        return ret
    ...
    class Meta:
        ...
        fields = ("display_id", "title", "description", "tags",
                  ...
                  "template", "spj", "rule_type", "source", "std_code")

class ExportObjectiveProblemSerializer(serializers.ModelSerializer):
    display_id = serializers.SerializerMethodField()
    description = serializers.SerializerMethodField()
    input_description = serializers.SerializerMethodField()
    output_description = serializers.SerializerMethodField()
    ...
    multiple_problem = serializers.SerializerMethodField()
    ...
    def get_option(self, obj):
        return [op for op in obj.option]

    def get_answer(self, obj):
        return [{'isAns':item['isAns'], 'option': item['option']} for item in obj.answer]
    ...
    class Meta:
        model = ObjectiveProblem
        fields = ("display_id", "title", "description", "tags",
                  "option", "hint", "answer", "source", "multiple_answer", "multiple_problem")
...
class AddContestObjectiveProblemSerializer(serializers.Serializer):
    contest_id = serializers.IntegerField()
...
class ExportObjectiveProblemRequestSerialzier(serializers.Serializer):
    objective_problem_id = serializers.ListField(child=serializers.IntegerField(), allow_empty=False)
...
class ImportProblemSerializer(serializers.Serializer):
    ...
    std_code = serializers.DictField(serializers.CharField(min_length=1))
