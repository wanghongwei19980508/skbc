...
from account.decorators import login_required, check_contest_permission, check_password
...
class ContestListAPI(APIView):
    def get(self, request):
        ...
        if lesson_id:
            contests = Contest.objects.select_related("created_by").filter(visible=True, lesson_id=lesson_id).order_by("start_time")
            return self.success(ContestSerializer(contests, many=True).data)
        else:
            contests = Contest.objects.select_related("created_by").filter(visible=True, lesson_id__isnull=True)
...
class ContestPasswordVerifyAPI(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            return self.error("Contest does not exist")
        if not check_password(data["password"], contest.password):
...
class ContestAccessAPI(APIView):
    ...
    def get(self, request):
        ...
        return self.success({"access": check_password(session_pass, contest.password)})
