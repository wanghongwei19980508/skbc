...
class ContestAPI(APIView):
    ...
    def post(self, request):
        ...
        if 'lesson_id' in data.keys():
            try:
                lesson = Lesson.objects.get(id=data.pop("lesson_id"))
    ...
    def get(self, request):
        ...
        if lesson_id:
            try:
                ...
                ensure_created_by(lesson, request.user)
                contests = Contest.objects.filter(lesson=lesson).order_by("-create_time")
            except Lesson.DoesNotExist:
                return self.error("Lesson does not exist")
        else:
            contests = Contest.objects.filter(lesson_id__isnull=True).order_by("-create_time")
