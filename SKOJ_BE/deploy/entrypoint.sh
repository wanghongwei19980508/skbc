...
mkdir -p $DATA/log $DATA/config $DATA/ssl $DATA/test_case $DATA/public/upload $DATA/public/avatar $DATA/public/website $DATA/public/slideshow $DATA/public/software
...
if [ ! -f "$DATA/public/website/logo.png" ]; then
    cp data/public/website/logo.png $DATA/public/website
fi

if [ ! -f "$DATA/public/website/nav_logo.png" ]; then
    cp data/public/website/nav_logo.png $DATA/public/website
fi
...
do
    ...
    python manage.py inituser --username=root --password=nicetry --action=create_super_admin &&
