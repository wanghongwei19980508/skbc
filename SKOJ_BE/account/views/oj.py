...
from ..serializers import (ApplyResetPasswordSerializer, ResetPasswordSerializer,
                           UserChangePasswordSerializer, UserLoginSerializer,UserChangePhoneSerializer,
                           UserRegisterSerializer, UsernameOrEmailOrPhoneCheckSerializer,
    ...
    def post(self, request):
        ...
        with open(os.path.join(settings.AVATAR_UPLOAD_DIR, name), "wb") as img:
            for chunk in avatar:
                img.write(chunk)
        user_profile = request.user.userprofile
        origin_path = os.path.join(settings.DATA_DIR,user_profile.avatar)
...
class CheckTFARequiredAPI(APIView):
    @validate_serializer(UsernameOrEmailOrPhoneCheckSerializer)
    def post(self, request):
        """
...
用户名、邮箱、手机号检查
'''
class UsernameOrEmailOrPhoneCheck(APIView):
    @validate_serializer(UsernameOrEmailOrPhoneCheckSerializer)
    def post(self, request):
        """
        ...
        if data.get("phone"):
            result["phone"] = User.objects.filter(phone=data["phone"]).exists()
...
class UserRegisterAPI(APIView):
    ...
    def post(self, request):
        ...
        user = User.objects.create(username=data["username"], email=data["email"], phone=data["phone"])
...
用户修改手机号接口
'''
class UserChangePhoneAPI(APIView):
    @validate_serializer(UserChangePhoneSerializer)
    @login_required
    def post(self, request):
        data = request.data
        user = auth.authenticate(username=request.user.username, password=data["password"])
        if user:
            if user.two_factor_auth:
                ...
                if not OtpAuth(user.tfa_token).valid_totp(data["tfa_code"]):
                    return self.error("Invalid two factor verification code")
            data["new_phone"] = data["new_phone"].lower()
            if User.objects.filter(phone=data["new_phone"]).exists():
                return self.error("The phone is owned by other account")
