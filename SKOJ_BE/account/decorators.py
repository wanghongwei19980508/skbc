...
from course.models import Course, CourseType, CourseStatus
from utils.api import JSONResponse, APIError
from utils.constants import CONTEST_PASSWORD_SESSION_KEY, COURSE_PASSWORD_SESSION_KEY
...
class lecturer_required(BasePermissionDecorator):
    def check_permission(self):
        user = self.request.user
        return user.is_authenticated and user.is_lecturer_role()
...
class school_manager_required(BasePermissionDecorator):
    def check_permission(self):
        user = self.request.user
        return user.is_authenticated and user.is_school_manager_role()

class teacher_researcher_required(BasePermissionDecorator):
    def check_permission(self):
        user = self.request.user
        return user.is_authenticated and user.is_teacher_researcher_role()
...
class problem_permission_required(admin_role_required):
    def check_permission(self):
        if not super(problem_permission_required, self).check_permission():
            return False
        if self.request.user.problem_permission == Permission.NONE:
            return False
        return True

class course_permission_required(admin_role_required):
    def check_permission(self):
        if not super(course_permission_required, self).check_permission():
            return False
        if self.request.user.course_permission == Permission.NONE:
            return False
        return True


def check_password(password, contest_password):
    ...
    def decorator(func):
        def _check_permission(*args, **kwargs):
            ...
            if self.contest.contest_type == ContestType.PASSWORD_PROTECTED_CONTEST:
                # password error
                if not check_password(request.session.get(CONTEST_PASSWORD_SESSION_KEY, {}).get(self.contest.id), self.contest.password):
...
def check_course_permission(check_type="details"):
    ...
    def decorator(func):
        def _check_permission(*args, **kwargs):
            ...
            if self.course.course_type == CourseType.PASSWORD_PROTECTED_COURSE:
                # password error
                if not check_password(request.session.get(COURSE_PASSWORD_SESSION_KEY, {}).get(self.contest.id), self.contest.password):
                    return self.error("Wrong password or password expired")

            # regular user get course problems, ranks etc. before course started
            if self.course.status == CourseStatus.COURSE_NOT_START and check_type != "details":
                return self.error("Course has not started yet.")

            if self.course.status == CourseStatus.COURSE_UNDERWAY:
                if check_type == "ranks" or check_type == "submissions":
                    return self.error(f"No permission to get {check_type}")
