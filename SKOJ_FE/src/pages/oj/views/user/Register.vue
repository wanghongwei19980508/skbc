...
export default {
  ...
  data() {
    const CheckUsernameNotExist = (rule, value, callback) => {
      api.checkUsernameOrEmailOrPhone(value, undefined, undefined).then(
        (res) => {
    ...
    const CheckEmailNotExist = (rule, value, callback) => {
      api.checkUsernameOrEmailOrPhone(undefined, value, undefined).then(
        (res) => {
    ...
    const CheckPhoneNotExist = (rule, value, callback) => {
      api.checkUsernameOrEmailOrPhone(undefined, undefined, value).then(
        (res) => {
          if (res.data.data.phone === true) {
            callback(new Error(this.$i18n.t("m.The_phone_already_exists")));
          } else {
            callback();
          }
        },
