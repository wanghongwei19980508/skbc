<template>
  <Row type="flex" :gutter="18">
    <Col :span="19">
      <Panel shadow>
        <div slot="title">{{ $t("m.Objective_Problem_List") }}</div>
        <div slot="extra">
          <ul class="filter">
            <li>
              <Dropdown @on-click="filterByType">
                <span
                  >{{
                    query.type === ""
                      ? this.$i18n.t("m.ObjectiveType")
                      : this.$i18n.t("m." + query.type)
                  }}
                  <Icon type="arrow-down-b"></Icon>
                </span>
                <Dropdown-menu slot="list">
                  <Dropdown-item name="">{{ $t("m.All") }}</Dropdown-item>
                  <Dropdown-item name="Radio">{{
                    $t("m.Radio")
                  }}</Dropdown-item>
                  <Dropdown-item name="Check">{{
                    $t("m.Check")
                  }}</Dropdown-item>
                  <Dropdown-item name="True_False">{{
                    $t("m.True_False")
                  }}</Dropdown-item>
                </Dropdown-menu>
              </Dropdown>
...
export default {
  ...
  data() {
    return {
      ...
      problemTableColumns: [
                ...
                on: {
                  click: () => {
                    this.$router.push({
                      ...
                      params: { objectiveProblemID: params.row._id },
                    });
                  },
                },
                style: {
        ...
        {
          title: this.$i18n.t("m.ObjectiveType"),
          render: (h, params) => {
            let t = params.row.type;
            let color = "blue";
            if (t === "Radio") color = "green";
            else if (t === "Check") color = "yellow";
            return h(
              "Tag",
              {
                props: {
  ...
  methods: {
    ...
    getObjectiveProblemList() {
      ...
      api.getObjectiveProblemList(offset, this.limit, this.query).then(
        (res) => {
    ...
    getTagList() {
      api.getObjectiveProblemTagList().then(
        (res) => {
    ...
    pickone() {
      api.pickoneobjective().then((res) => {
        this.$success("Good Luck");
        this.$router.push({
          name: "objective-problem-details",
          params: { objectiveProblemID: res.data.data },
        });
      });
    },
  },
