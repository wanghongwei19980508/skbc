            ...
            <li>
              <Dropdown @on-click="handleResultChange">
                ...
                <Dropdown-menu slot="list">
                  ...
                  <Dropdown-item
                    v-for="status in Object.keys(OBJECTIVE_PROBLEM_STATUS)"
                    :key="status"
                    :name="status"
                  >
                    {{
                      $t(
                        "m." +
                          OBJECTIVE_PROBLEM_STATUS[status].name.replace(
                            / /g,
                            "_"
                          )
                      )
...
import { OBJECTIVE_PROBLEM_STATUS, USER_TYPE } from "@/utils/constants";
import utils from "@/utils/utils";
import time from "@/utils/time";
import Pagination from "@/pages/oj/components/Pagination";

export default {
  name: "objectiveSubmissionList",
  components: {
    Pagination,
  },
  data() {
            ...
            if (params.row.show_link) {
              return h(
                "span",
                {
                  ...
                  on: {
                    click: () => {
                      this.$router.push("/objective-status/" + params.row.id);
                    },
                  },
                },
        ...
        },
        {
          ...
          render: (h, params) => {
            return h(
              ...
              {
                props: {
                  color: OBJECTIVE_PROBLEM_STATUS[params.row.result].color,
                },
              },
              this.$i18n.t(
                "m." +
                  OBJECTIVE_PROBLEM_STATUS[params.row.result].name.replace(
                    / /g,
                    "_"
                  )
              )
                  ...
                  click: () => {
                    if (this.contestID) {
                      this.$router.push({
                        ...
                        params: {
                          objectiveProblemID: params.row.objective_problem,
  ...
  mounted() {
    this.init();
    this.OBJECTIVE_PROBLEM_STATUS = Object.assign({}, OBJECTIVE_PROBLEM_STATUS);
  },
  methods: {
    init() {
      this.contestID = this.$route.params.contestID;
      let query = this.$route.query;
      this.objectiveProblemID = query.objectiveProblemID;
    ...
    getObjectiveSubmissions() {
      ...
      params.objective_problem_id = this.objectiveProblemID;
      let offset = (this.page - 1) * this.limit;
      let func = this.contestID
        ? "getContestObjectiveSubmissionList"
        : "getObjectiveSubmissionList";
    ...
    handleRejudge(id, index) {
      ...
      api.objectiveSubmissionRejudge(id).then(
        (res) => {
  ...
  computed: {
    ...
    title() {
      if (!this.contestID) {
        ...
        return this.$i18n.t("m.Objective_Problem_Submissions");
      } else {
        return this.$i18n.t("m.Objective_Submissions");
      }
    },
    status() {
      return this.formFilter.result === ""
        ...
        : this.$i18n.t(
            "m." +
              OBJECTIVE_PROBLEM_STATUS[this.formFilter.result].name.replace(
                / /g,
                "_"
              )
          );
