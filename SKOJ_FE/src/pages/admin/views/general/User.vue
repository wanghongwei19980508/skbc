  ...
  <div class="view">
    <Panel :title="$t('m.User_User')">
      ...
      <el-table v-loading="loadingTable" element-loading-text="loading" @selection-change="handleSelectionChange"
        ...
        <el-table-column prop="phone" :label="$t('m.Phone')"></el-table-column>
    ...
    <Panel>
      <span slot="title">{{ $t("m.Import_User") }}
        <el-popover placement="right" trigger="hover">
          <p>
            Only support csv file without headers, 要求csv文件不带头, 共四列:
            用户名，密码，邮箱，真实姓名。
          </p>
      ...
      <template v-else>
        <el-table :data="uploadUsersPage">
          ...
          <el-table-column :label="$t('m.RealName')">
            <template slot-scope="{ row }">
              {{ row[3] }}
            </template>
          </el-table-column>
        ...
        <div class="panel-options">
          <el-button type="primary" size="small" icon="el-icon-fa-upload" @click="handleUsersUpload">{{ $t('m.Import_All')
          }}
          </el-button>
          <el-button type="warning" size="small" icon="el-icon-fa-undo" @click="handleResetData">{{ $t('m.Reset_Data') }}
          </el-button>
    ...
    <Panel :title="$t('m.Generate_User')">
      <el-form :model="formGenerateUser" ref="formGenerateUser">
        ...
        <el-form-item>
          <el-button type="primary" @click="generateUser" icon="el-icon-fa-users"
            :loading="loadingGenerate">{{ $t('m.Generate_And_Export') }}
          </el-button>
          <span class="userPreview" v-if="
    ...
    <el-dialog :title="$t('m.User_Info')" :visible.sync="showUserDialog" :close-on-click-modal="false">
      <el-form :model="user" label-width="120px" label-position="left">
        <el-row :gutter="20">
          ...
          <el-col :span="12">
            <el-form-item :label="$t('m.User_Phone')" required>
          ...
          <el-col :span="12">
            <el-form-item :label="$t('m.User_Type')">
              <el-select v-model="user.admin_type">
                ...
                <el-option :label="$t('m.Lecturer')" value="Lecturer"></el-option>
                <el-option :label="$t('m.School_Manager')" value="School Manager"></el-option>
                <el-option :label="$t('m.Teacher_Researcher')" value="Teacher Researcher"></el-option>
          ...
          <el-col :span="12">
            <el-form-item :label="$t('m.Problem_Permission')">
              <el-select v-model="user.problem_permission" :disabled="user.admin_type === 'Regular User'">
          ...
          <el-col :span="12">
            <el-form-item :label="$t('m.Course_Permission')">
              <el-select v-model="user.course_permission" :disabled="
                user.admin_type === 'Regular User' ||
                user.admin_type === 'Lecturer'
              ">
                <el-option :label="$t('m.None')" value="None"></el-option>
                <el-option :label="$t('m.Own')" value="Own"></el-option>
                <el-option :label="$t('m.All')" value="All"></el-option>
          ...
          <el-col :span="8">
            <el-form-item :label="$t('m.Two_Factor_Auth')">
              <el-switch v-model="user.two_factor_auth" :disabled="!user.real_tfa" active-color="#13ce66"
...
export default {
        ...
        api
          ...
          .then((res) => {
            ...
            utils.downloadFile(url).then(() => {
              this.$alert(
                "All users created successfully, the users sheets have downloaded to your disk.",
                "Notice"
              );
            });
            this.getUserList(1);
  ...
  watch: {
    ...
    "user.admin_type"() {
      ...
      if (this.user.admin_type === "Super Admin") {
        this.user.course_permission = "All";
      } else if (this.user.admin_type === "Regular User") {
        this.user.course_permission = "None";
      } else if (this.user.admin_type === "Lecturer") {
        this.user.course_permission = "None";
      } else {
        this.user.course_permission = "Own";
      }
    },
