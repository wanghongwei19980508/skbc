  ...
  <div>
    <panel>
      ...
      <el-table :data="data">
        ...
        <el-table-column prop="id" :label="$t('m.Test_Case_ID')">
        </el-table-column>
        <el-table-column :label="$t('m.Option')" fixed="right" width="200">
          <template slot-scope="{row}">
            <icon-btn :name="$t('m.Delete')" icon="trash" @click.native="deleteTestCase(row.id)"></icon-btn>
          </template>
        </el-table-column>
      </el-table>
      <div class="panel-options" v-show="data.length > 0">
        <el-button type="warning" size="small" :loading="loading" icon="el-icon-fa-trash"
          @click="deleteTestCase()">{{ $t('m.Delete_All') }}
        </el-button>
      </div>
    </panel>
  </div>
