  ...
  <div class="view">
    <Panel :title="$t('m.Contest_List')">
      ...
      >
        ...
        <el-table-column :label="$t('m.Contest_Type')" width="180">
          <template slot-scope="scope">
            <el-tag
              :type="
        ...
        <el-table-column fixed="right" width="275" :label="$t('m.Operation')">
          <div slot-scope="scope">
            ...
            ></icon-btn>
            <icon-btn
              :name="$t('m.Objective_Problem')"
              icon="list-ol"
              @click.native="
                goContestObjectiveProblemList(scope.row.id, scope.row.title)
              "
            ></icon-btn> -->
            <icon-btn
              :name="$t('m.Public_Topic')"
              icon="list-ol"
              @click.native="
                goContestPublicTopicList(scope.row.id, scope.row.title)
              "
            ></icon-btn>
            <icon-btn
              :name="$t('m.Announcement')"
              icon="info-circle"
              @click.native="goContestAnnouncement(scope.row.id)"
            ></icon-btn>
            <icon-btn
              icon="download"
              :name="$t('m.Download_Accepted_Submissions')"
...
export default {
  ...
  methods: {
    ...
    //   });
    // },
    // goContestObjectiveProblemList(contestId, contestTitle) {
    //   this.$router.push({
    //     name: "contest-objective-problem-list",
    //     params: { contestId },
    //   });
    // },
    goContestPublicTopicList(contestId, contestTitle) {
      this.$router.push({
