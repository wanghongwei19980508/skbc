  ...
  <div class="problem">
    <Panel :title="title">
      <el-form ref="form" :model="problem" :rules="rules" label-position="top" label-width="70px">
        <el-row :gutter="20">
          <el-col :span="6">
            <el-form-item prop="_id" :label="$t('m.Display_ID')" :required="this.routeName === 'create-contest-problem' ||
        ...
        <el-row :gutter="20">
          ...
          <el-col :span="8">
            <el-form-item :label="$t('m.Tag')" :error="error.tags" required>
              <span class="tags">
                <el-tag v-for="tag in problem.tags" :closable="true" :close-transition="false" :key="tag" type="success"
        ...
        <el-row>
          ...
          <el-col :span="11">
            <el-form-item :label="$t('m.Instance_Code')">
              <el-col :span="24" v-for="(v, k) in std_code" :key="'std_code' + k">
                <el-form-item>
                  <el-checkbox v-model="v.checked">{{ k }}</el-checkbox>
                  <div v-if="v.checked">
                    <code-mirror v-model="v.code" :mode="v.mode"></code-mirror>
        ...
        <el-form-item :label="$t('m.Special_Judge')" :error="error.spj">
          <el-col :span="24">
            <el-checkbox v-model="problem.spj" @click.native.prevent="switchSpj()">{{ $t("m.Use_Special_Judge")
            }}</el-checkbox>
          </el-col>
        </el-form-item>
...
export default {
  ...
  data() {
    return {
      rules: {
        ...
        input_description: {
          required: true,
          message: this.$i18n.t("m.Input_Description_Is_Required"),
          trigger: "blur",
        },
        output_description: {
          required: true,
          message: this.$i18n.t("m.Output_Description_Is_Required"),
          trigger: "blur",
        },
      },
  ...
  watch: {
    ...
    "problem.languages"(newVal) {
      ...
      if (languages.includes("C++") && languages[0] != "C++") {
        languages[0] = "C++";
        languages[1] = "C";
      }
      for (let item of languages) {
        if (this.template[item] === undefined) {
          ...
          if (this.problem.std_code[item] === undefined) {
            std_code[item] = {
              checked: false,
              code: langConfig.config.template,
              mode: langConfig.content_type,
            ...
            std_code[item] = {
              checked: true,
              code: this.problem.std_code[item],
              mode: langConfig.content_type,
            };
          }
        } else {
          data[item] = this.template[item];
          std_code[item] = this.std_code[item];
        }
      }
      this.template = data;
  ...
  methods: {
    switchSpj() {
      if (this.testCaseUploaded) {
        this.$confirm(this.$i18n.t('m.If_You_Change_Problem_Judge_Method') + ',' + this.$i18n.t('m.You_Need_To_Re_Upload_Test_Cases'), "Warning", {
    ...
    submit() {
      if (!this.problem.samples.length) {
        this.$error(this.$i18n.t('m.Sample_Is_Required'));
        return;
      }
      for (let sample of this.problem.samples) {
        if (!sample.input || !sample.output) {
          this.$error(this.$i18n.t('m.Sample_Input_And_Output_Is_Required'));
          return;
        }
      }
      ...
      if (this.problem.spj) {
        if (!this.problem.spj_code) {
          this.error.spj = this.$i18n.t('m.Spj_Code_Is_Required');
        } else if (!this.problem.spj_compile_ok) {
          this.error.spj = this.$i18n.t('m.SPJ_Code_Has_Not_Been_Successfully_Compiled');
        }
        if (this.error.spj) {
      ...
      if (!this.problem.languages.length) {
        this.error.languages = this.$i18n.t('m.Please_Choose_At_Least_One_Language_For_Problem');
        this.$error(this.error.languages);
        return;
      }
      if (!this.testCaseUploaded) {
        this.error.testCase = this.$i18n.t('m.Test_Case_Is_Not_Uploaded_Yet');
      ...
      if (this.problem.rule_type === "OI") {
        for (let item of this.problem.test_case_score) {
          try {
            ...
            this.$error(this.$i18n.t('m.Test_Case_Score_Must_Be_An_Integer'));
            return;
          }
        }
      }
      ...
      if (
        this.problem.languages.includes("C++") &&
        this.problem.languages[0] != "C++"
      ) {
        this.problem.languages[0] = "C++";
        this.problem.languages[1] = "C";
      }
      ...
      this.problem.std_code = {};
      for (let k in this.std_code) {
        if (this.std_code[k].checked) {
          this.problem.std_code[k] = this.std_code[k].code;
        }
      }
      let funcName = {
