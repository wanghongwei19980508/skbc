  ...
  <div class="view">
    <Panel :title="this.$i18n.t('m.Problem_List')">
      <div slot="title" v-if="contestId">
        <el-page-header @back="goBack" :content="this.$i18n.t('m.Contest_Problem_List')" style="margin-top: 5px">
        </el-page-header>
      </div>
      <div slot="header">
        ...
        <el-table-column fixed="right" :label="$t('m.Operation')" width="250">
          <div slot-scope="scope">
            ...
            <icon-btn v-if="contestId" :name="$t('m.Make_Public')" icon="clone"
              @click.native="makeProblemPublic(scope.row.id)"></icon-btn>
    ...
    <el-dialog :title="$t('m.Sure_To_Update_The_Problem')" width="20%" :visible.sync="InlineEditDialogVisible"
      ...
      <span slot="footer">
        ...
        getProblemList(currentPage);"></cancel>
        <save @click.native="updateProblem(currentRow)"></save>
      </span>
    </el-dialog>
    <el-dialog :title="$t('m.Add_Contest_Problem')" v-if="contestId" width="80%" :visible.sync="addProblemDialogVisible"
      @close-on-click-modal="false">
