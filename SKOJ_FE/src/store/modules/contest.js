...
const mutations = {
  ...
  [types.CHANGE_CONTEST_OBJECTIVE_PROBLEMS](state, payload) {
    state.contestObjectiveProblems = payload.contestObjectiveProblems
  },
...
const actions = {
  ...
  getContestObjectiveProblems({ commit, rootState }) {
    return new Promise((resolve, reject) => {
      api.getContestObjectiveProblemList(rootState.route.params.contestID).then(res => {
        res.data.data.sort((a, b) => {
          if (a._id === b._id) {
            return 0
          } else if (a._id > b._id) {
        ...
        res.data.data.map((data) => {
          data.problem_type = '客观题'
        })
        commit(types.CHANGE_CONTEST_OBJECTIVE_PROBLEMS, { contestObjectiveProblems: res.data.data })
        resolve(res)
      }, () => {
        commit(types.CHANGE_CONTEST_OBJECTIVE_PROBLEMS, { contestObjectiveProblems: [] })
      })
    })
  },
